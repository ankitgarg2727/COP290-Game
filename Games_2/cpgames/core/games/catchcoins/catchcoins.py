...
from .modules import Hero, Food, ShowEndGameInterface,StartInterface,ShowEndGameInterface_two
...
class CatchCoinsGame(PygameBaseGame):
    ...
    def run(self):
        ...
        while flag:
            ...
            hero = Hero(game_images['wet'], position=(375, 520))
            ...
            while True: 
                ...
                while True:            
                    ...
                    if (pygame.time.get_ticks()-start_time) % 30000 >= 10000:
                        current_left=hero.rect.left
                        current_top=hero.rect.top
                        break
                    pygame.display.flip()
                    clock.tick(cfg.FPS)
                if(gameended==True):
                    break  
                hero = Hero(game_images['dry'], position=(current_left, current_top))    
                while True:
                    screen.fill(0)
                    screen.blit(game_images['background'], (0, 0))
                    countdown_text = 'Count down: ' + str((90000 - (pygame.time.get_ticks()-start_time)) // 60000) + ":" + str((90000 - (pygame.time.get_ticks()-start_time)) // 1000 % 60).zfill(2)
                ...
                if(gameended==True):
                    break       
                hero = Hero(game_images['hazardous'], position=(current_left, current_top))
                while True:
                    screen.fill(0)
                    screen.blit(game_images['background'], (0, 0))
                    countdown_text = 'Count down: ' + str((90000 - (pygame.time.get_ticks()-start_time)) // 60000) + ":" + str((90000 - (pygame.time.get_ticks()-start_time)) // 1000 % 60).zfill(2)
                ...
                if(gameended==True):
                    break    
                hero = Hero(game_images['wet'], position=(current_left, current_top))   
